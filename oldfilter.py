...
def calculate_price(user_data, gym_data):
    ...
    if user_data["trainer_type"] == "super_off_peak":
        key = "Super-off peak (10 am - 12 pm & 2 pm - 4 pm)" if user_data["subscribe"] == "yes" else "Super-off peak (10 am - 12 pm & 2 pm - 4 pm)"
        price += gym_data.get(key, 0)
    elif user_data["trainer_type"] == "off_peak":
        key = "Off-peak (12 - 2 pm & 8 - 11 pm)" if user_data["subscribe"] == "yes" else "Off-peak (12 - 2 pm & 8 - 11 pm)"
        price += gym_data.get(key, 0)
    elif user_data["trainer_type"] == "anytime":
        key = "Anytime" if user_data["subscribe"] == "yes" else "Anytime"
        price += gym_data.get(key, 0)
...
def calculate_price_with_discount(user_data, gym_data):
    ...
    if age <= 25 or age >= 66:  # Student/Young Adult and Pensioners discount
        if user_data["Massage_Therapy"] != "yes" and user_data["Physiotherapy"] != "yes":  # Exclude massage therapy and physiotherapy from discount
            discount_percent = 20 if age <= 25 else 15  # 20% discount for students/young adults, 15% for pensioners
...
user_data = {
    ...
    "subscribe": form.getvalue("subscribe", "no"),  # Include subscription status
...
# Calculate the total price for Power Zone with discount
...
# Calculate the total price for uGym with discount
